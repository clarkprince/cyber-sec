...
class DatasourceCard extends TWElement {
  ...
  slotFormElement: Ref<HTMLSlotElement> = createRef();

  hasOrganizingNotebooks: boolean;
  ...
  replace() {
    ...
    for (const elt of inputs(
      this.slotFormElement.value!.assignedElements({ flatten: true }),
    )) {
  ...
  async submit(e: SubmitEvent) {
    ...
    let action = (e.currentTarget! as HTMLButtonElement).value;
  ...
  render() {
    ...
    return html`
      ...
      >
        ...
        ${when(
          this.isEdited,
          this.editView.bind(this),
          this.defaultView.bind(this),
        )}
      </div>
    `;
  }
