...
class Editor extends TWElement {
    constructor() {
        ...
        this.firstUpdated = () => {
            ...
            // TODO: track this promise for a loading mode
            // we shouldn't allow other operations while data re being read
    ...
    nodeInSelection(node) {
        // TODO: sometimes an error there is no node(1)
        return this.edstate.selection.$from.node(1).type == node;
    }
    ...
    render() {
        ...
        for (const mt of [schema.marks.strong, schema.marks.em, schema.marks.underline]) {
            buttons.push(html `
          <button class="h-8 w-8 flex items-center justify-center hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 first:rounded-l"
                  active=${this.markInSelection(mt)}
      ...
      `);
        ...
        // colour
        buttons.push(html `
      <button class="h-8 w-8 flex items-center justify-center hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600"
              active=${this.showColourMenu}
              @click="${() => this.toggleColourMenu()}">
        ${icons["colour"]}
      </button>
    `);
        // titles
        buttons.push(html `
      <button class="h-8 w-8 flex items-center justify-center hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600"
              active=${this.showFontSizeMenu}
              @click="${() => this.toggleFontSizeMenu()}">
        ${icons["title"]}
      </button>
    `);
        // TODO:
        // example using either a directive or a custom built-in
        // the custom built-in is clearly cleaner but require a Safari polyfill
        // (un)ordered list
        for (const mt of [schema.nodes.bullet_list, schema.nodes.ordered_list]) {
            buttons.push(html `
          <button class="h-8 w-8 flex items-center justify-center hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 last-of-type:rounded-r"
                  active=${this.nodeInSelection(mt)}
      ...
      `);
        ...
        colours = html `
      <ul class=${classMap({
            "list-none pl-0 mb-0 absolute top-full left-24 z-10 translate-y-2.5 bg-neutral-200 dark:bg-yellow-400 rounded": true,
            "invisible": this.showmenu != menu_machine.tooltip || !this.showColourMenu
        })}>
        <li class="mb-1">
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 rounded-t text-black text-left"
                  active=${this.markInSelection(schema.marks.colour_red)}
                  @click="${() => this.changeColour(schema.marks.colour_red)}">
            Red
          </button>
        </li>
        <li class="mb-1">
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 text-black text-left"
                  active=${this.markInSelection(schema.marks.colour_yellow)}
                  @click="${() => this.changeColour(schema.marks.colour_yellow)}">
            Yellow
          </button>
        </li>
        <li class="mb-1">
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 text-black text-left"
                  active=${this.markInSelection(schema.marks.colour_green)}
                  @click="${() => this.changeColour(schema.marks.colour_green)}">
            Green
          </button>
        </li>
        <li class="mb-1">
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 text-black text-left"
                  active=${this.markInSelection(schema.marks.colour_black)}
                  @click="${() => this.changeColour(schema.marks.colour_black)}">
            Black
          </button>
        </li>
        <li>
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 rounded-b text-black text-left"
                  active=${this.markInSelection(schema.marks.colour_white)}
                  @click="${() => this.changeColour(schema.marks.colour_white)}">
            White
          </button>
        </li>
      </ul>
    `;
      ...
      <ul class=${classMap({
        ...
        <li class="mb-1">
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 rounded-t text-black text-left"
                  active=${this.headingInSelection(1)}
                  @click="${() => this.makeHeadline(1)}">
            Heading 1
          </button>
        </li>
        <li class="mb-1">
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 text-black text-left"
                  active=${this.headingInSelection(2)}
                  @click="${() => this.makeHeadline(2)}">
            Heading 2
          </button>          
        </li>
        <li class="mb-1">
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 text-black text-left"
                  active=${this.headingInSelection(3)}
                  @click="${() => this.makeHeadline(3)}">
            Heading 3
          </button>
        </li>
        <li>
          <button class="px-2 py-0.5 w-full hover:bg-neutral-300 active:bg-neutral-400 dark:hover:bg-yellow-500 dark:active:bg-yellow-600 rounded-b text-black text-left"
                  active=${this.headingInSelection(0)}
                  @click="${() => this.makeHeadline(0)}">
            Text
          </button>
        </li>
      </ul>
    `;
      ...
      <div
          ...
          class=${classMap({
            "flex absolute z-20 -translate-y-full bg-neutral-200 dark:bg-yellow-400 rounded": true,
