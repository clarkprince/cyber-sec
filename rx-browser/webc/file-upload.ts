...
export function filesAttached(ev: DragEvent): Array<File> {
  ...
  if (ev.dataTransfer.items) {
    return [...ev.dataTransfer.items]
      ...
      .filter((f) => !!f) as Array<File>; // TS not infering filter
  } else {
...
export async function uploadFile(e: DragEvent, notebook: string): Promise<any> {
  e.preventDefault();
  e.stopPropagation();
  ...
  const res = await fetch(
    `/api/datasource?action=save&type=upload&title=${file.name}&notebook=${notebook}`,
    {
      method: "POST",
      body: formData,
    },
