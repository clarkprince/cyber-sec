...
class PivotPill extends TWElement {
  ...
  /** Must be defined if loading is false */
  pivot: PivotDragData;
  loading: boolean;
  /** Optionally display the pivotid during loading */
  pivotid: string;
  /** Optionally pass some styling class */
  cls: string;
  ...
  render() {
    if (this.loading) {
      return html`<div
        class=${`inline-block mx-1 py-1 border-2 dark:border border-zinc-300 text-xs rounded cursor-pointer px-1 ${
          this.cls || ""
        }`}
    ...
    const leftOp = on
      ...
      : nothing;
    return html`<div
      class=${`inline-block border-2 dark:border border-zinc-300 text-xs rounded-sm cursor-pointer ${
        this.cls || ""
      }`}
      draggable="true"
