...
import {
  DatasourceDef,
  getCellDragData,
  getDatasourceDragData,
  hasCellData,
  hasDatasourceData,
  setCellDragData,
} from "./notebook-body-events";
import { notify } from "./notification";
import TWElement from "./tw";
import { ManifestChangeEvent, manifestChangeEvt } from "./manifest-events";
import "./pivot-pill";
import icons from "./icons";
import { AppliedPivot, Cell, Playbook } from "./models";
import { fetchNotebook, fetchPivot } from "./api";

type Panel = "none" | "sources" | "search";

async function saveNotebook(updatedNotebook: Playbook) {
  ...
  if (rsp.status !== 200) {
    notify(
      updatedNotebook.ID,
      "the notebook could not be saved, retrying usually helps",
      true,
    );
  }
}

function isDrop(ev: Event): ev is DragEvent {
  return ev.type === "drop";
}
...
class NotebookBody extends TWElement {
  ...
  notebookId: string;
  notebook: Playbook;
  // read-only fields computed by the server template
  owner: string;
  created: string;
  // other state
  panel: Panel = "none";
  /** Native JS Map are bad for reactivity (not immutable) hence the object */
  pivots: { [pivotId: string]: AppliedPivot } = {};

  /** hold a ref to rendered cells to allow scrolling/focusing to them */
  cellRefs: Map<string, Ref<HTMLElement>>;

  /** track which cell menu is opened, using the cell ID */
  openedCellMenu: string = "";

  // ts trick to avoid mistakenly using "this.title" (which exists for HTML element)
  //  instead of "this.notebook.Title"
  title: never;

  // opens/closes the snapshot submenu
  snapshotMenu: boolean = false;
  snapshots: string[] = [];
  adjustCellFlag: boolean = false;
  ...
  firstUpdated() {
    ...
    this.shadowRoot?.addEventListener(manifestChangeEvt, (ev) => {
      const manifest: Cell = JSON.parse((ev as ManifestChangeEvent).detail);
  ...
  async loadPivot(p: AppliedPivot) {
    const pivot = await fetchPivot(p.Pivot);
    if (!pivot) return;
    this.pivots = { ...this.pivots, [pivot.pivotid]: { ...pivot, ...p } };
  }

  async cellDataFromDrop(ev: DragEvent): Promise<DatasourceDef | null> {
    if (hasCellData(ev)) {
      // duplicating a cell
      const cell = getCellDragData(ev);
      if (!cell?.DataSource) return null;
      return {
      ...
      const data = await getDatasourceDragData(
        ev as DragEvent,
        this.notebook.ID,
      );
      if (!data) {
        ...
        console.warn(
          "Dropped something that is not a datasource definition on notebook-body",
        );
        return null;
      }
      return data;
   ...
   */
  async createCell(ev: DragEvent | CustomEvent) {
    ev.preventDefault();
    let data: any;
    if (isDrop(ev)) {
  ...
  async removeCell(cell: string | null) {
    if (!cell) return;
  ...
  async adjustCell(ev: MouseEvent, cell: Cell, action: string) {
    if (!cell) return;
  ...
  moveCell(cell: Cell, direction: number) {
    ...
    if (this.isValidRange(index, direction, cells.length)) {
      ...
      const neighbors = cells.filter(
        (c) => c.Position.Row === cell.Position.Row + direction,
      )!;
      if (neighbors.length === 0) {
        if (
          (cell.Position.Row === 1 && direction === -1) ||
  ...
  isValidRange(index: number, direction: number, cellCount: number): boolean {
    return (
      index >= 0 && index + direction >= 0 && index + direction < cellCount
    );
  }

  swapCells(cells: Cell[], index1: number, index2: number, direction: number) {
    if (
      index1 < 0 ||
      index1 >= cells.length ||
      index2 < 0 ||
  ...
  adjustRowForCells(cells: Cell[], startIndex: number, direction: number) {
    for (let i = startIndex; i < cells.length; i++) {
      cells[i].Position.Row += direction;
    }
  }

  adjustWidth(cell: Cell, ev: MouseEvent) {
    if (!cell.Position.Col || cell.Position.Col === 0) cell.Position.Col = 3;
    cell.Position.Col =
  ...
  adjustHeight(cell: Cell, ev: MouseEvent) {
    ...
    if (index >= 0) {
      if (ev.ctrlKey || ev.metaKey) {
        if (index < ranges.length - 1) cell.Position.Height = ranges[index + 1];
      } else if (index > 0) {
  ...
  async exportNotebook() {
    ...
    const rsp = await fetch(
      `/api/nbar?action=export&notebook=${this.notebookId}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async fetchSnapshots(cell: Cell | null) {
    if (!cell) return;
    this.snapshots = [
      "Loading.."
    ];
    const rsp = await fetch(
      `/api/notebook/snapshot?action=list`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async saveSnapshot(cell: Cell | null) {
  ...
  render() {
    if (!this.notebook) return; // waiting for state to be initialized

    document.title = this.notebook.Title;

    const adjustCellMenu = async (cell: Cell) => {
    ...
    const snapshotAndEnforce = async (cell: Cell) => {
        ...
        if (cell.ID) {
          ...
          return html`<div class="${cellWidth}" aria-label="cell-wrapper">
            ...
            >
              ...
              <button
                @click=${() => {
                  if (this.openedCellMenu == cell.ID) this.openedCellMenu = "";
                  else this.openedCellMenu = cell.ID!;
                }}
