...
import { EventsCodes, } from "./datacell-abi";
import { locateEntity } from "./coordinates";
import { getPivotDragData, removePivotDragImage, setPivotDragData, setPivotDragImage, hasPivotData, } from "./pivot-events";
        ...
        if (isClick(event) || isRightClick(event) || isDoubleClick(event)) {
            ...
            switch (event.code) {
                ...
                case "F10":
                    ...
                    event.preventDefault();
                    this.passEvent(EventsCodes.ShowDebugMenu, this.shadowRoot.querySelector("[id]"));
                default:
                    return;
            }
        }
            ...
            const entity = elem.closest("[id]");
            const [q, _t] = await new Promise((continuation) => this.passEvent(EventsCodes.DragStart, entity, { continuation }));
