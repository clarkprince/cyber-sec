...
class DatasourceReplace extends TWElement {
  ...
  saveNotebook() {
    if (this.isAudit) {
      // send to audit
      this.dispatchEvent(
        new CustomEvent("replace-datasource", {
          detail: {
  ...
  replaceDatasource(c: any, ev: any) {
        ...
        cells.push(
          ...
          >
            ...
            <select
              ...
              @input="${(ev: any) => this.replaceDatasource(cell.ID, ev)}"
            >
              <option value="">Select Datasource</option>
              ${this.datasourcelist.map(
                (ds: any) =>
                  html`<option value="${ds.id}">${ds.title}</option>`,
              )}
            </select>
          </div>`,
        );
