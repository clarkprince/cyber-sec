...
// Prefix returns the first 63 bits of the ULID as a positive integer, suitable for use as a table identifier.
// This relies on having a low number of entries created within a short period of time (safe for most entities).
func (id ULID) Prefix() int64 {
	return int64(id[0])<<55 | int64(id[1])<<47 | int64(id[2])<<39 | int64(id[3])<<31 | int64(id[4])<<23 | int64(id[5])<<15 | int64(id[6])<<7 | int64(id[7])>>1
}

// Marshal returns the base32 encoding of the given prefix.
// It is always the case that:
//
//	strings.HasPrefix(id.String(), MarshalPrefix(id.Prefix()))
func MarshalPrefix(pfx int64) string {
	dst := make([]byte, 12)
	dst[0] = Encoding[byte(pfx>>60)&31]
	dst[1] = Encoding[byte(pfx>>55)&31]
	dst[2] = Encoding[byte(pfx>>50)&31]
	dst[3] = Encoding[byte(pfx>>45)&31]
	dst[4] = Encoding[byte(pfx>>40)&31]
	dst[5] = Encoding[byte(pfx>>35)&31]
	dst[6] = Encoding[byte(pfx>>30)&31]
	dst[7] = Encoding[byte(pfx>>25)&31]
	dst[8] = Encoding[byte(pfx>>20)&31]
	dst[9] = Encoding[byte(pfx>>15)&31]
	dst[10] = Encoding[byte(pfx>>10)&31]
	dst[11] = Encoding[byte(pfx>>5)&31]
...
func (id ULID) Equal(other ULID) bool { return bytes.Equal(id[:], other[:]) }
...
func (id ULID) IsZero() bool { return zeroValue.Compare(id) == 0 }

func (id *ULID) Set(v string) error {
	vu, err := ParseStrict(v)
	if err != nil {
		return err
	}
	*id = vu
