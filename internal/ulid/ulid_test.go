...
// we don’t rely on anything specific about ULID, so rand is good enough there
func TestULID_Prefix(t *testing.T) {
	for i := 0; i < 2000; i++ {
		var id ULID
		_, err := crand.Read(id[:])
		if err != nil {
		...
		if full, pfx := id.String(), MarshalPrefix(id.Prefix()); !strings.HasPrefix(full, pfx) {
			t.Errorf("Invalid prefix: want %s, got %s", full, pfx)
		}
	}
