...
func (a *Archive) AppendFile(name string, data []byte) {
...
// GetLines returns non-empty lines at name
func (a *Archive) GetLines(name string) []string {
	ls := strings.Split(a.Get(name), "\n")
	j := 0
	for i := range ls {
		if len(ls[i]) > 0 {
			ls[j] = ls[i]
